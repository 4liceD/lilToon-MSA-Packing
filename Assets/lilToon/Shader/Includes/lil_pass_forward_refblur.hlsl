...
float4 frag(v2f input LIL_VFACE(facing)) : SV_Target
{
    ...
    if (_Packing == 1) {
        fd.smoothness *= LIL_SAMPLE_2D_ST(_SmoothnessTex, lil_sampler_linear_repeat, fd.uvMain).g;
    }
    if (_Packing == 2) {
        fd.smoothness *= LIL_SAMPLE_2D_ST(_SmoothnessTex, lil_sampler_linear_repeat, fd.uvMain).a;
    }
    #endif
